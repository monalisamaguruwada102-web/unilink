...
// --- UNI-LINK STUDENT ECOSYSTEM (v2.0 PRODUCTION) ---
...
export default function App() {
  ...
  const fetchDiscovery = async () => {
    ...
    if (isStudyMode) query = query.eq('course', userData.course); // Filter for Study Buddy Mode
  ...
  if (!session && (view === 'register' || view === 'login')) return (
    <SafeAreaView className="flex-1 bg-[#05051e] p-8 justify-center items-center">
       ...
       <TouchableOpacity onPress={() => setView('register')} className="mt-8"><Text className="text-white/20 text-[10px] font-black uppercase tracking-widest italic tracking-[0.4em]">Register New Account</Text></TouchableOpacity>
    </SafeAreaView>
  );
       ...
       <View className="flex-1 pb-32">
          ...
          {view === 'feed' && (
             <ScrollView className="flex-1 px-8 pt-4">
                ...
                <View className="bg-white/5 p-5 rounded-[2.5rem] mb-12 border border-white/5 flex-row items-center gap-4 shadow-xl">
                   <TextInput className="flex-1 text-white text-sm font-bold italic" placeholder={isAnonPost ? "Type yourPoly student confession..." : "Post to your campus flow..."} placeholderTextColor="#333" />
                ...
                {posts.map(p => (
                   <View key={p.id} className={`p-8 rounded-[3.5rem] border mb-8 shadow-2xl ${p.is_anonymous ? 'bg-black border-white/20' : 'bg-white/5 border-white/5'}`}>
                      <View className="flex-row items-center gap-4 mb-6">
                         ...
                         <View className="flex-1"><Text className="text-white font-black text-sm italic">{p.is_anonymous ? 'Secret Student Word' : p.profiles?.name}</Text><Text className="text-[10px] text-white/20 uppercase tracking-widest font-black leading-none">{p.profiles?.polytechnic} • CAMPUS NEWS</Text></View>
                      </View>
             ...
             <ScrollView className="flex-1 px-8 pt-4">
                ...
                {matches.length > 0 ? matches.map(m => (
                    <TouchableOpacity key={m.id} className="bg-white/5 p-6 rounded-[2.5rem] border border-white/5 flex-row items-center gap-4 mb-4 relative overflow-hidden">
                       ...
                       <View className="flex-1"><Text className="text-white font-black italic uppercase text-sm mb-1">{m.profiles?.name || 'New Match'}</Text><Text className="text-[9px] text-white/20 font-black uppercase tracking-[0.2em]">{m.profiles?.polytechnic} • MATCHED</Text></View>
                  ...
                  <View className="items-center justify-center pt-24 opacity-30 text-center"><Star color="#7c3aed" size={64}/><Text className="text-sm font-black uppercase mt-6 text-white text-center italic tracking-widest leading-relaxed">Matches & Messages will appear here after mutual Poly interest!</Text></View>
                )}
             </ScrollView>
          )}
          ...
          {view === 'profile' && (
            <View className="flex-1 px-10 pt-10 items-center">
               ...
               <Text className="text-primary-400 font-bold tracking-[0.3em] text-[10px] uppercase mt-4">{userData.poly || 'Zimbabwe Poly'}</Text>
               <View className="w-full bg-white/5 p-8 rounded-[3rem] border border-white/5 mt-12"><Text className="text-[9px] font-black uppercase text-primary-400 tracking-widest mb-3 italic tracking-[0.2em]">Student Academic Record</Text><Text className="text-sm font-black text-white italic uppercase">{userData.course} • Year {userData.year}</Text></View>
